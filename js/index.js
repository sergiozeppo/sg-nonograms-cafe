...
const sketch = (p, id) => {
  ...
  function resize() {
    p.resizeCanvas(
      zoom * (2 * margin + (maxHorHints + numCols) * cellSize),
  ...
  function resetGrid() {
    ...
    gridHorHints = [];
    for (let hints of horHints) gridHorHints.push(Array(hints.length).fill(0));

    gridVerHints = [];
    for (let hints of verHints) gridVerHints.push(Array(hints.length).fill(0));
  ...
  function drawGrid() {
    for (let row = 0; row < numRows; row++) {
      for (let col = 0; col < numCols; col++) {
        ...
        let cellColor =
          cellColors[
            2 * ((Math.floor(row / 5) + Math.floor(col / 5)) % 2) +
              ((row + col) % 2)
          ];
  ...
  function drawPosHighlight() {
    ...
    if (mouseInfo.inGridY && (mouseInfo.inGridX || mouseInfo.inHintsX)) {
      p.rect(
        -maxHorHints * cellSize - 1,
        mouseInfo.row * cellSize - 1,
        (maxHorHints + numCols) * cellSize + 2,
        cellSize + 2
      );
    }
    ...
    if (mouseInfo.inGridX && (mouseInfo.inGridY || mouseInfo.inHintsY)) {
      p.rect(
        mouseInfo.col * cellSize - 1,
        -maxVerHints * cellSize - 1,
        cellSize + 2,
        (maxVerHints + numRows) * cellSize + 2
      );
    }
  }
  ...
  function getEventInfo(x, y, type, button = -1) {
    ...
    return { type, button, col, row, inGridX, inGridY, inHintsX, inHintsY };
  }
  ...
  p.touchStarted = function (event) {
    ...
    if (p.touches.length == 1) handleClickEvent(getTouchInfo(p.touches[0]));
    return false;
  };

  p.touchMoved = function (event) {
    if (event.target !== canvas.elt) return;
    if (p.touches.length == 1) handleDragEvent(getTouchInfo(p.touches[0]));
    return false;
  };
  ...
  function handleClickEvent(eventInfo) {
    if (eventInfo.inGridX) {
      if (eventInfo.inGridY) clickInGrid(eventInfo);
      else if (eventInfo.inHintsY) clickInVerHints(eventInfo);
  ...
  p.mouseDragged = function (event) {
    if (event.target !== canvas.elt) return;
    if (actionEvent && actions.length > 0) handleDragEvent(getMouseInfo());
  };
  ...
  function handleDragEvent(eventInfo) {
    ...
    if (action.type == ACTION_TYPE.MARK_CELL) {
      if (eventInfo.inGridX && eventInfo.inGridY) {
        ...
        if (
          (eventInfo.type == EVENT.TOUCH && currVal != action.to) ||
          (action.from == CELL_MARK.EMPTY && currVal == CELL_MARK.EMPTY) ||
          (action.to == CELL_MARK.EMPTY && currVal == action.from) ||
          (action.from != CELL_MARK.EMPTY &&
            action.to != CELL_MARK.EMPTY &&
            currVal != action.to)
        )
          addAction({
            type: ACTION_TYPE.MARK_CELL,
            row: eventInfo.row,
            col: eventInfo.col,
            from: currVal,
            to: action.to,
      ...
      if (
        ...
        if (col < numHints) {
          ...
          if (currVal == action.from)
            addAction({
              type: ACTION_TYPE.TOGGLE_HOR_HINT,
              row: eventInfo.row,
              index: index,
              from: currVal,
            });
      ...
      if (
        ...
        if (row < numHints) {
          ...
          if (currVal == action.from)
            addAction({
              type: ACTION_TYPE.TOGGLE_VER_HINT,
              col: eventInfo.col,
              index: index,
              from: currVal,
            });
  ...
  function clickInGrid(eventInfo) {
    ...
    if (eventInfo.type == EVENT.MOUSE) {
      if (eventInfo.button == p.LEFT)
        nextVal =
          currVal == CELL_MARK.BLACK ? CELL_MARK.EMPTY : CELL_MARK.BLACK;
      else if (eventInfo.button == p.RIGHT)
        nextVal =
          currVal == CELL_MARK.WHITE ? CELL_MARK.EMPTY : CELL_MARK.WHITE;
  ...
  function undo() {
    ...
    for (let action of currActions) unapply(action);
  ...
  function redo() {
    ...
    for (let action of currActions) apply(action);
  ...
  function displaySecretMessage() {
    ...
    if (msgType == 0) msgDiv.textContent = code;
    else msgDiv.appendChild(getAnchor(nono.getSteamGiftsURL(code)));
  ...
  function isSolved() {
    ...
    for (let row = 0; row < numRows; row++) {
      ...
      for (let col = 0; col <= numCols; col++) {
        if (col == numCols || grid[row][col] != CELL_MARK.BLACK) {
          // White or after last cell
          if (currCount > 0) {
    ...
    for (let col = 0; col < numCols; col++) {
      ...
      for (let row = 0; row <= numRows; row++) {
        if (row == numRows || grid[row][col] != CELL_MARK.BLACK) {
          // White or after last cell
          if (currCount > 0) {
            if (currGroup >= numHints || hints[currGroup] != currCount) {
              return false;
  ...
  p.keyPressed = function () {
    if (
      ...
      (p.key === "y" && p.keyIsDown(p.CONTROL))
    ) {
      redo();
    } else if (
      p.keyCode === p.LEFT_ARROW ||
      (p.key === "z" && p.keyIsDown(p.CONTROL))
    ) {
      undo();
